...
if [ ! -f "$(pwd)/.env" ]; then
    ...
    if [ ! -z "$PROJECTNAME" ]; then break; fi; done
    ...
    if [ ! -z "$MYSQL_ROOT_PASSWORD" ]; then break; fi; done
    ...
    while true; do
        ...
        if [ -z "$PHPMYADMIN_RESTRICTION" ]; then
    ...
    if [ ! -z "$BASE_DOMAIN" ] && [ "$BASE_DOMAIN" =~ \.lvh\.me$ ]; then break; fi; done

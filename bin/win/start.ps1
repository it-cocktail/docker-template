...
for %%P in (%*) do (
    SET PARAMETER=%%P
    if "%PARAMETER%" == "-d" (
        SET ADDITIONAL_CONFIGFILE=%ADDITIONAL_CONFIGFILE% -f docker-data/config/base/docker-compose.debug.yml
...
if [%JAVA_SRC_FOLDER%] NEQ [] (
    if exist %JAVA_SRC_FOLDER%\nul (
        if "%DEBUGMODE%"=="1" (
            SET ADDITIONAL_CONFIGFILE=%ADDITIONAL_CONFIGFILE% -f docker-data/config/base/docker-compose.java.yml -f docker-data/config/base/docker-compose.debug_java.yml
            set JAVADEBUGENABLED=1
            set JAVA_VIRTUAL_HOST=java.%BASE_DOMAIN%, java.%SECONDARY_DOMAIN%
        ) else (
            SET ADDITIONAL_CONFIGFILE=%ADDITIONAL_CONFIGFILE% -f docker-data/config/base/docker-compose.java.yml
        )
...
docker-compose -p "%PROJECTNAME%" -f docker-data/config/base/docker-compose.yml %ADDITIONAL_CONFIGFILE% pull 2>&1 | findstr /R "^Status ^Pulling"
